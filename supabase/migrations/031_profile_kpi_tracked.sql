-- Payroll-only staff (labour, telecallers, etc.) skip KPI tracking but stay on payroll.
ALTER TABLE public.profiles
  ADD COLUMN IF NOT EXISTS kpi_tracked boolean NOT NULL DEFAULT true;

CREATE INDEX profiles_kpi_tracked_idx
  ON public.profiles (kpi_tracked)
  WHERE kpi_tracked = true;
